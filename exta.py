...
# Always ensure mapping_debug.txt exists in the correct debug_logs folder
try:
    open(os.path.join(DEBUG_DIR, "mapping_debug.txt"), "a", encoding="utf-8").close()
except Exception:
    pass
...
def scroll_dropdown_to_end(region, scroll_anchor_abs, desired_value=None):
    ...
    for attempt in range(DROPDOWN_MAX_SCROLLS):
        lines = ocr_region_lines(region, debug_name=f"dropdown_scroll_a{attempt}")
...
class TTSEngine:
    def __init__(self, rate=175, volume=1.0):
        self.engine = pyttsx3.init(driverName="sapi5")  # Windows
...
def fuzzy_map_kv(kv, canonical_list):
    ...
    # logging mapping decisions for debugging
    try:
        with open(os.path.join(DEBUG_DIR, "mapping_debug.txt"), "a", encoding="utf-8") as mf:
            mf.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - fuzzy_map_kv result: {mapped}\n")
    except Exception:
        pass
    return mapped
...
_ANNUAL_INCOME_CANON = set(DROPDOWN_OPTIONS.get("Annual Income", {}).get("options", []))
def _append_info_panel_raw(pass_name: str, raw_text: str):
    """
    Appends raw PaddleOCR joined text into a single history file.
    This makes tracing OCR errors very easy per pass / per run.
    """
    try:
        os.makedirs(DEBUG_DIR, exist_ok=True)
        path = os.path.join(DEBUG_DIR, "info_panel_raw_history.txt")
...
def fill_annual_income_dropdown_strict(field_meta, field_abs_pos, desired_value) -> bool:
    ...
    for attempt in range(DROPDOWN_MAX_SCROLLS * 3):
        check_pause()
        lines = ocr_region_lines(region, debug_name=f"AnnualIncome_FIX_{attempt}")
...
def _micro_verify_text(region_abs, box_local, required_text, tol_casefold=True):
    ...
    lines = ocr_region_lines(patch, debug_name=None)
...
def wait_for_button_and_click(label, region, timeout=30, poll_interval=0.6,
    ...
    """Continuously OCR a region and click when a button label is detected.

    Uses reasoning_core.normalize_text + fuzzy similarity + token overlap
    to be robust to OCR noise.

    If click_pos is provided, we use that coordinate for the click once the label
    ...
    while time.time() < deadline:
        ...
        # 🔍 DEBUG: See what OCR is actually reading (only a few times)
        ...
        # Threshold is intentionally relaxed a bit
        if best_line is not None and best_score >= 0.55:
            # We detected something that looks like the button
            if click_pos:
                # Use the coordinate the user marked in learning mode
                cx, cy = click_pos
                pyautogui.moveTo(cx, cy, duration=0.12)
                pyautogui.click()
            else:
                # Prefer the existing verified_click_option for stability
                if not verified_click_option(region, best_line, label):
                    # Fallback: simple center click on the detected line
...
def run_upload_and_next_form(mem):
    """Click Upload → OK → wait for Take Screenshot → wait for Load Another Form.

    Returns:
        True  → we clicked Load Another Form (expect a new form to appear)
        False → something failed; caller should stop the multi-form loop.
    """
    ...
    # As soon as we click Upload, reset debug logs
    try:
        if os.path.isdir(DEBUG_DIR):
            shutil.rmtree(DEBUG_DIR)
        os.makedirs(DEBUG_DIR, exist_ok=True)
        print("[FLOW] debug_logs folder reset after upload click.")
    except Exception as e:
        print("[FLOW] Failed to reset debug_logs:", e)

    # 2) Click OK within ~2 seconds
    deadline = time.time() + 2.0
    while time.time() < deadline:
        check_pause()
        click_abs(ok_pos[0], ok_pos[1])
        break  # single click is enough

    # 3) Wait for Take Screenshot button to appear in the marked region
    print("[FLOW] Waiting for 'Take Screenshot' button…")
    ts_ok = wait_for_button_and_click(
        "Take Screenshot",
        flow_region,
        timeout=40,
        poll_interval=0.7,
        debug_prefix="take_screenshot",
        click_pos=ts_pos,
    )

    # 🔁 FALLBACK: if OCR failed but we have a stored position, try timed clicks
    if not ts_ok and ts_pos:
        print("[FLOW] OCR could not confidently detect 'Take Screenshot'. Using coordinate fallback.")
        for i in range(6):
            check_pause()
            click_abs(ts_pos[0], ts_pos[1])
            safe_sleep(1.0)
        # Assume one of these clicks triggered the screenshot
        ts_ok = True

    if not ts_ok:
        speak("Could not detect Take Screenshot button. Stopping upload loop.")
        return False

    # 4) After clicking Take Screenshot, the UI loads again.
    #    We keep watching the SAME region until Load Another Form appears.
    print("[FLOW] Waiting for 'Load Another Form' button…")
    la_ok = wait_for_button_and_click(
        "Load Another Form",
        flow_region,
        timeout=60,
        poll_interval=0.7,
        debug_prefix="load_another_form",
        click_pos=la_pos,
    )

    # 🔁 FALLBACK: same idea for Load Another Form
    if not la_ok and la_pos:
        print("[FLOW] OCR could not confidently detect 'Load Another Form'. Using coordinate fallback.")
        for i in range(10):
            check_pause()
            click_abs(la_pos[0], la_pos[1])
            safe_sleep(1.0)
        la_ok = True

    if not la_ok:
        speak("Could not detect Load Another Form button. Stopping upload loop.")
        return False

    # Give the UI a moment to load the next form
    safe_sleep(2.5)
...
def _district_first_precise_probe(region, scroll_anchor_abs, desired_value,
    ...
    for probe in range(2):
        check_pause()
        lines = ocr_region_lines(region, debug_name=f"District_PROBE_{probe}")
...
def _heavy_dropdown_fast_phase(field_name, region, scroll_anchor_abs, desired_value,
    ...
    while fast_ticks < profile["FAST_TICKS"]:
        check_pause()
        # OCR visible page
        lines = ocr_region_lines(region, debug_name=f"{field_name}_FAST_{fast_ticks}")
...
def _heavy_dropdown_precise_phase(field_name, region, scroll_anchor_abs, desired_value,
    ...
    while ticks < profile["MAX_TOTAL_TICKS"]:
        check_pause()
        lines = ocr_region_lines(region, debug_name=f"{field_name}_PRECISE_{ticks}")
...
def _stabilize_exact_match(region, target_text, passes=3, prefer_y=None):
    ...
    for i in range(max(2, passes)):
        lines = ocr_region_lines(region, debug_name=f"lock_{tnorm}_{i}")
        hits = []
        for ln in lines:
...
# --- NEW: route heavy lists to specialized handler ---
    ...
    for attempt in range(DROPDOWN_MAX_SCROLLS * 9):
        ...
        if lname == "education":
            ...
            lines = ocr_region_lines(region, debug_name=f"dropdown_fast_{field_name}_{attempt}")
...
def precise_dropdown_scan(field_name, region, scroll_anchor_abs, target, forward=True):
    ...
    for attempt in range(DROPDOWN_MAX_SCROLLS * 6):
        ...
        if (field_name or "").strip().lower() == "education":
            ...
            lines = ocr_region_lines(region, debug_name=f"dropdown_precise_{field_name}_{attempt}")
        ...
        if run_reasoning:
            ...
            # Extra Education-only trace (so you can see WHY it didn't click)
            if lname == "education":
                try:
                    os.makedirs(DEBUG_DIR, exist_ok=True)
                    fp = os.path.join(DEBUG_DIR, "education_reasoning_trace.txt")
                    with open(fp, "a", encoding="utf-8") as f:
                        f.write("\n" + "=" * 70 + "\n")
                        f.write(f"attempt={attempt} forward={forward}\n")
                        f.write(f"target={target!r} target_norm={target_norm!r}\n")
                        f.write(f"reason_status={reason.get('status')} conf={reason.get('confidence')}\n")
                        f.write(f"reason_visible={reason.get('visible_text')!r} canonical={reason.get('canonical')!r}\n")
                        f.write("visible_lines:\n")
                        for ln0 in lines:
                            t0 = (ln0.get('text') or '').strip()
                            if not t0:
                                continue
                            f.write(f"  - {t0!r}\n")
                except Exception:
        ...
        # Education-only trace to debug match failures
        if lname == "education":
            try:
                os.makedirs(DEBUG_DIR, exist_ok=True)
                fp = os.path.join(DEBUG_DIR, "education_precise_match_trace.txt")
                with open(fp, "a", encoding="utf-8") as f:
                    f.write("\n" + "-" * 70 + "\n")
                    f.write(f"attempt={attempt} forward={forward} target={target!r} target_norm={target_norm!r}\n")
            except Exception:
                pass
            ...
            if lname == "education":
                if verified_click_option_edu(region, ln, raw_txt):
                    ...
                    try:
                        ...
                        fp = os.path.join(DEBUG_DIR, "education_precise_match_trace.txt")
                        with open(fp, "a", encoding="utf-8") as f:
                            f.write(f"MATCH_CLICKED: raw={raw_txt!r} norm={n_raw!r}\n")
                    except Exception:
                        pass
...
def parse_dob(value_str):
    ...
    # Log for debugging
    try:
        with open(os.path.join(DEBUG_DIR, "fill_failures.txt"), "a", encoding="utf-8") as f:
            f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - DOB parse failed for '{value_str}'\n")
    except Exception:
        pass
...
def fill_dob_by_memory(field_meta, value_str):
    ...
    # Debug log (optional)
    try:
        with open(os.path.join(DEBUG_DIR, "dob_typed.txt"), "a", encoding="utf-8") as f:
            f.write(
                f"{time.strftime('%Y-%m-%d %H:%M:%S')} - "
                f"Typed DOB: {day_str}/{month_str}/{year_str} (raw='{value_str}')\n"
            )
    except Exception:
        pass
...
def upload_learning_mode_only():
    """Mini learning mode:
    Capture Upload Details button, OK button, Take Screenshot button,
    Load Another Form button, and the upload-flow region.

    Uses existing memory (fields, anchors, etc. are not touched).
    """
    mem = load_memory()
    if not mem:
    ...
    speak("I will capture Upload Details, OK button, Take Screenshot, Load Another Form and the upload-flow region.")
    ...
    print("Hover over 'Take Screenshot' and press 3")
    keyboard.wait('3')
    ts = pyautogui.position()
    mem['take_screenshot_button_pos'] = [ts.x, ts.y]

    # 4) Load Another Form button
    speak("Now hover over the 'Load Another Form' button and press 4.")
    print("Hover over 'Load Another Form' and press 4")
    keyboard.wait('4')
    la = pyautogui.position()
    mem['load_another_form_button_pos'] = [la.x, la.y]

    # 5) Region where loading + buttons appear
    speak("Now mark the region where loading happens and where these buttons appear.")
    speak("Hover the TOP-LEFT corner of that area and press 5.")
    print("Hover TOP-LEFT of upload-flow region and press 5")
    keyboard.wait('5')
    tl = pyautogui.position()

    speak("Hover the BOTTOM-RIGHT corner of the same area and press 6.")
    print("Hover BOTTOM-RIGHT of upload-flow region and press 6")
    keyboard.wait('6')
    br = pyautogui.position()
    ...
    speak("Upload Details, OK, Take Screenshot, Load Another Form and region captured successfully.")
    print("Upload Details, OK, Take Screenshot, Load Another Form and region captured successfully.")

    save_memory(mem)
    speak("Upload-only learning completed and memory saved. You can now run Autofill Mode.")
    print("Upload-only learning completed and memory saved.")
        ...
        try:
            ...
            if not mapped:
                if paddle_text and paddle_text.strip():
                    ...
                    with open(os.path.join(DEBUG_DIR, "info_panel_text_auto.txt"), "w", encoding="utf-8") as f:
                        f.write(paddle_text)
                ...
                with open(os.path.join(DEBUG_DIR, "INFO_PANEL_PADDLE_FAILED.txt"), "w", encoding="utf-8") as f:
                    f.write("PaddleOCR returned empty text. No fallback used.\n")
            ...
            with open(os.path.join(DEBUG_DIR, "info_panel_text_auto.txt"), "w", encoding="utf-8") as f:
                for k in mapped:
                    f.write(f"{k}: {mapped[k]}\n")

            return mapped

        except Exception as e:
            print("[PADDLE INFO OCR ERROR]", e)
        with open(os.path.join(DEBUG_DIR, "INFO_PANEL_PADDLE_EXCEPTION.txt"), "w", encoding="utf-8") as f:
            f.write(f"PaddleOCR exception: {e}\nNo fallback used.\n")
        return {}   # or: raise
...
def read_info_panel_full_before_autofill(mem) -> dict:
    ...
    try:
        ...
        try:
            with open(os.path.join(DEBUG_DIR, "info_panel_merged_final.txt"), "w", encoding="utf-8") as f:
...
def interactive_info_snip_fallback():
    ...
    with open(os.path.join(DEBUG_DIR, "info_panel_text_fallback.txt"), "w", encoding="utf-8") as f:
        f.write(raw)
...
def fill_pada_dropdown_strict(field_meta, field_abs_pos, desired_value) -> bool:
    ...
    for attempt in range(DROPDOWN_MAX_SCROLLS * 2):
        check_pause()
        lines = ocr_region_lines(region, debug_name=f"pada_fix_{attempt}")
...
def fill_diet_dropdown_strict(field_meta, field_abs_pos, desired_value) -> bool:
    ...
    for attempt in range(DROPDOWN_MAX_SCROLLS * 3):
        check_pause()
        lines = ocr_region_lines(region, debug_name=f"diet_fix_{attempt}")
        for ln in lines:
            raw = (ln.get('text') or "").strip()
            if not raw:
                continue
...
def _wait_for_info_panel(mem, last_signature, form_index,
    ...
    """
    Wait for the info panel to show a (new) form using OLD OCR only.
    Returns: (ok, signature)
    """
    deadline = time.time() + (max_wait_first if form_index == 0 else max_wait_new)
...
def merge_info_dict(base: dict, incoming: dict) -> dict:
    ...
    for k, v in incoming.items():
        ...
        if _norm_v(a) in _norm_v(v):
            ...
            # keep base, just log
            try:
                with open(os.path.join(DEBUG_DIR, "info_panel_merge_conflicts.txt"), "a", encoding="utf-8") as f:
                    f.write(f"{k}: '{a}'  VS  '{v}'\n")
            except Exception:
                pass
    return base
...
if __name__ == "__main__":
    ...
    speak("Bot ready. Press F2 to start Learning Mode. Press F3 for Autofill Mode. Press F4 for Upload-Only Learning.")
    print("F2 -> Learning Mode, F3 -> Autofill Mode, F4 -> Upload-Only Learning. F5 to skip optional captures where shown.")
